...
loginForm.addEventListener("submit", async (event) => {
  ...
  const loginEmailInput = document.querySelector(
    '.form-wrapper.sign-in input[type="text"]'
  );
  const loginPasswordInput = document.querySelector(
    '.form-wrapper.sign-in input[type="password"]'
  );
  const loginEmail = loginEmailInput.value;
  const loginPassword = loginPasswordInput.value;

  try {
    const response = await fetch("https://backend-psi-lemon.vercel.app/login", {
      method: "POST",
      headers: {
...
signupForm.addEventListener("submit", async (event) => {
  ...
  const signupEmailInput = document.querySelector(
    '.form-wrapper.sign-up input[type="email"]'
  );
  const signupPasswordInput = document.querySelector(
    '.form-wrapper.sign-up input[type="password"]'
  );
  ...
  try {
    const response = await fetch(
      "https://backend-psi-lemon.vercel.app/signup",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
